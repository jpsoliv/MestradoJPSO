...
Filtros ativos podem ser classificados em dois tipos: os baseados em amplificadores operacionais e os baseados em conversores CC-CA. O princípio de funcionamento do primeiro tipo de filtro é igual em sistema passivo, onde a operação é dada pela atenuação de determinadas componentes de frequência da rede. A diferença entre esse tipo de filtro ativo com os passivos é a presença de amplificadores operacionais, a qual necessitam de fontes externas para funcionar adequadamente. Esse tipo de filtragem é muito utilizado em sistemas de baixa potência, sendo que para o escopo desse trabalho torna-se inviável dado o tipo de potência do sistema elétrico de aeronaves, sendo assim não será abordado nesse trabalho. Já a filtragem ativa tem por princípio a utilização de conversores CC-CA, visto que este tipo de conversor pode recriar formas de tensão e corrente de qualquer configuração dada à uma referência \cite{Pomilio2009}. O princípio dos filtros ativos é prover um alto fator de potência, seja pela distorção e/ou de deslocamento, com a injeção de corrente na rede que, somado com a requerida pela carga, faz com que esta apresente uma forma de onda senoidal e em fase com a tensão. Isso é conseguido aplicando uma fonte de corrente e/ou tensão controladas nas linhas da rede do sistema. Estas fontes são concebidas por inversores de frequência cujo controle é feito pela leitura da tensão de alimentação e corrente de entrada na carga, gerando na saída uma forma de onda compatível com a correção do fator de potência do sistema.
