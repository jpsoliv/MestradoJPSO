...
Em sistemas ativos os conversores são operados com a implementação de chaves estáticas com comutação controlada cujo objetivo é de diversificar as topologias de circuitos em comparação com os sistemas apenas comutados por diodos. Com isso, a flexibilidade de projeto é aumentada de modo que há uma maior diversificação de tipos de retificadores disponíveis. Além do mais, com esse controle é possível haver uma melhor regulação da tensão na saída do sistema e um controle de corrente de entrada de modo a proporcionar um fator de potência compatível com o sistema.

Existe também a utilização de conversores inversores para atuar como filtros ativos. O controle da intensidade da tensão/corrente na saída desse tipo de conversor pode ser utilizado para injetar corrente no sistema de modo a anular as harmônicas presentes no sistema.
