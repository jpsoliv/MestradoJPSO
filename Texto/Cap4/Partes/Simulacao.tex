...
A simulação contempla a operação de três EHAs operando simultaneamente. O perfil da amplitude de corrente da carga com os três EHA atuando concomitantemente é ilustrado na Figura \ref{fig:perfil.eps}. Essa corrente é solicitada com o deslocamento linear do pistão do atuador indo de uma extremidade a outra com a aplicação de uma carga mecânica em seu eixo.
...
\begin{figure*}[!htb] %Circuito típico de um retificador de 12 pulsos com sua respectiva corrente de entrada
	...
	\end{subfigure}%
	\caption{Formas de onda de tensão e corrente em regime de Máxima Corrente}
	\label{fig:8}
\end{figure*}
...
\begin{figure*}[!htb] %Circuito típico de um retificador de 12 pulsos com sua respectiva corrente de entrada
	...
	\end{subfigure}%
	\caption{Potências instantâneas em regime de Máxima Corrente}
	\label{fig:9}
\end{figure*}
...
\begin{figure*}[!htb] %Circuito típico de um retificador de 12 pulsos com sua respectiva corrente de entrada
	...
	\end{subfigure}%
	\caption{Detalhe das potências instantâneas em regime de Máxima Corrente}
	\label{fig:10}
\end{figure*}
...
\begin{figure*}[!htb] %Circuito típico de um retificador de 12 pulsos com sua respectiva corrente de entrada
	...
	\end{subfigure}%
	\caption{Espectro de frequência da tensão em regime de Máxima Corrente e delimitação definida pela norma MIL-STD 704F}
	\label{fig:11}
\end{figure*}
...
\begin{figure*}[!htb] %Circuito típico de um retificador de 12 pulsos com sua respectiva corrente de entrada
	...
	\end{subfigure}%
	\caption{Amplitude das componentes harmônicas em regime de Máxima Corrente}
	\label{fig:12}
\end{figure*}
...
As formas de onda no período de regime permanente é mostrado na Figura \ref{fig:19}. Aqui pode ser observado que a corrente mantém-se estável até cessar no final do intervalo, a qual varia entre 73,7 e 73.9 segundos.

As observações proferidas nesse período são semelhantes às verificadas durante o regime transitório, isto é, apesar de a forma de onda da corrente apresentar-se como distorcida, esta não suficiente para elevar as distorções da tensão de modo a prover resultados não condizentes com a MIL-STD 704F. Contudo, pode-se observar (Figura \ref{fig:resultados_filt_20.eps}) que o filtro ainda atua nesse sentido, tornando a corrente mais próxima de uma senoide e elevando a qualidade de energia.
