...
\begin{table}[!htb]
	...
	\label{tab:Zgen}
\end{table}
\todo[backgroundcolor=red!25]{De onde vem esses valores?? REF FEITO}
...
O modelo é composto por diversos blocos criados para cada função de um filtro. A teoria de potência instantânea apresentadas no Capítulo \ref{cap:Filtros Ativos para Sistemas Elétricos} será implementada em um desses blocos para realizar os cálculos das correntes de referências do compensador. O compensador será implementada em outro bloco segundo as definições apresentadas no Capítulo \ref{cap:Característica de Filtros Ativos em Sistemas Reais}. A operação conjunta do compensador com o controle concebe o filtro ativo como todo.
...
\end{figure}
\todo[backgroundcolor=red!25]{Nunca use o termo "medição" para descrever uma estimativa FEITO}
